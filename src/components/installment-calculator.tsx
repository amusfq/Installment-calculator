...
export function InstallmentCalculatorComponent({currentMonth}: Props) {
  ...
  const calculateInstallment = (principal: number) => {
    ...
    const totalAmount = principal * (1 + margin)
    const monthlyInstallment = totalAmount / 10 // 10 months
    return monthlyInstallment
  }

  const marginAmount = principal * 0.35
  const monthlyInstallment = calculateInstallment(principal)
  ...
  const installments = Array.from({ length: 10 }, (_, index) => {
    ...
    return {
      ...
      balance: principal + marginAmount - monthlyInstallment * (index + 1)
    }
  })
          ...
          {principal > 0 && (
            <div className="mt-6">
              ...
              <div className="mb-4 space-y-2">
                ...
                <p>
                  <strong>Cicilan Per Bulan:</strong> {formatIDR(monthlyInstallment)}
                </p>
                <p>
                  <strong>Total Pembayaran:</strong>{" "}
                  {formatIDR(principal + marginAmount)}
                </p>
              </div>
